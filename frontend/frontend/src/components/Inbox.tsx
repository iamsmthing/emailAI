...
const InboxComponent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchOutlookMails = async () => {
      const accessToken = token_ms;
    
      try {
        const response = await axios.post('http://localhost:4000/auth/fetchOutlookMails', {}, {
          headers: {
...
const fetchGmailEmails = async () => {
  ...
  try {
    const response = await axios.post('http://localhost:4000/auth/fetchGmailEmails', {}, {
      headers: {
        Authorization: `Bearer ${accessToken}`, // Send accessToken in the header
        'Content-Type': 'application/json',
      },
    ...
    const fetchEmails = async () => {
      ...
      const gmailEmails = token ? await fetchGmailEmails() : {};
      const outlookEmails = token_ms ? await fetchOutlookMails() : {};
  ...
  <div className="space-y-4 bg-gray-800 px-6 pb-6 rounded-lg shadow-lg h-[90vh] overflow-y-auto">
    ...
    {Object.keys(emails).length > 0 ? (
      Object.keys(emails).map((author) => (
        <div
          key={author}
          className={`bg-slate-700 p-4 rounded-lg shadow border-l-4 ${
                    ...
                    {email.source === 'Gmail' && (
                      <p
                        className={`text-sm font-semibold ${
                          checkReadStatus(email.labelIds) === 'UNREAD'
                            ? 'text-red-400'
                            : 'text-emerald-500'
                        }`}
                      >
